...
type Props = {
  product: Product;
  onDelete: Function; //para usar a função onDelete - outro componente
};
...
const ProductCrudCard = ({ product, onDelete }: Props) => {
  ...
  const handleDelete = (productId: number) => {
    ...
    if (!window.confirm('Tem certeza que deseja deletar?')) {
      //popup - se clicar no botão cancelar 
      return;
    }

    //fazer requisção no backend em productId com o método DELETE
    ...
    //passando para a requisição
    requestBackend(config).then(() => {
      //chama função onDelete
      onDelete();
